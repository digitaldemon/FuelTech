...
function makeMusic(muted: boolean) {
  ...
  return {
    ...
    close: () => { if (fadeTimer) clearInterval(fadeTimer); fadeTimer = null; audio.pause(); audio.src = ''; },
  };
}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '28px 18px 40px', gap: 0 }}>
          ...
          <div style={{ width: '100%', maxWidth: 960 }}>
            ...
            <div style={{ background: '#0c1524', border: '1px solid rgba(255,255,255,0.08)', borderTop: '1px solid rgba(255,255,255,0.04)', borderRadius: '0 0 14px 14px', padding: '10px 15px 13px', display: 'flex', flexDirection: 'column', gap: 9 }}>
              <div ref={progressRef} className="prog-bar" onClick={e => { if (!progressRef.current) return; const r = progressRef.current.getBoundingClientRect(); seekTo(((e.clientX - r.left) / r.width) * TOTAL); }} onMouseMove={e => { if (!progressRef.current) return; const r = progressRef.current.getBoundingClientRect(); const t = ((e.clientX - r.left) / r.width) * TOTAL; const idx = SCENES.findIndex(s => t >= s.start && t < s.end); setHoveredChapter(idx === -1 ? null : idx); }} onMouseLeave={() => setHoveredChapter(null)}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
                  {/* Mute toggle */}
                  <button className="ctrl-btn" title={muted ? 'Unmute' : 'Mute'} onClick={() => { setMuted(m => { const next = !m; mutedRef.current = next; if (soundRef.current) { soundRef.current.close?.(); soundRef.current = makeSounds(next); } return next; }); }}>
                    {muted
