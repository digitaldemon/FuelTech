...
function App() {
  ...
  }), tab === "nrfi" && /*#__PURE__*/React.createElement(FirstInning, null), tab === "analyze" && /*#__PURE__*/React.createElement(Analyze, {
    fw: fw,
    onSave: saveEntry,
    pending: pending,
  ...
  if (path.indexOf("tennis") === 0) {
    ...
    events.forEach(ev => {
      ...
      comps.forEach((comp, ci) => {
        ...
        rows.push({
          eventId: String(comp.id || ev.id + "-" + ci),
          path,
...
async function pitcherFirstInning(pid, season) {
  ...
  let val = null;
  try {
    const d = await getJson("https://statsapi.mlb.com/api/v1/people/" + pid + "/stats?stats=statSplits&group=pitching&sitCodes=i01&season=" + season);
    ...
    if (st && st.gamesPlayed) {
      ...
      val = {
        rate: Number(st.runs || 0) / st.gamesPlayed,
        sample: st.gamesPlayed,
        era: st.era != null ? Number(st.era) : null,
        whip: st.whip != null ? Number(st.whip) : null,
        k9,
        bb9,
        hr9,
        innings: ip,
...
async function teamOffenseSplits(teamId, season) {
  ...
  let val = null;
  try {
    const d = await getJson("https://statsapi.mlb.com/api/v1/teams/" + teamId + "/stats?stats=statSplits&group=hitting&sitCodes=i01,vr,vl&season=" + season);
    const splits = d.stats && d.stats[0] && d.stats[0].splits || [];
    const find = re => {
      const s = splits.find(x => re.test(x.split && x.split.description || ""));
      return s && s.stat;
    };
    const i01 = find(/first inning/i),
      vr = find(/right/i),
      vl = find(/left/i);
    if (i01 && i01.gamesPlayed) {
      val = {
        rate: Number(i01.runs || 0) / i01.gamesPlayed,
        sample: i01.gamesPlayed,
...
const parseIp = ip => {
  ...
  return Number(m[0] || 0) + (m[1] === "1" ? 1 / 3 : m[1] === "2" ? 2 / 3 : 0);
};
async function pitcherMeta(pid, season) {
  ...
  let hand = null,
    ...
    allow = null;
  try {
    const [p, gl] = await Promise.all([getJson("https://statsapi.mlb.com/api/v1/people/" + pid + "?hydrate=stats(group=[pitching],type=[season],season=" + season + ")"), getJson("https://statsapi.mlb.com/api/v1/people/" + pid + "/stats?stats=gameLog&group=pitching&season=" + season)]);
...
function platoonFactor(off, oppHand) {
  if (!off || !oppHand || off.opsVsR == null || off.opsVsL == null) return {
    f: 1,
    note: "platoon data n/a"
  };
  ...
  return {
    f: nClamp(ops / base, 0.85, 1.18),
    note: "OPS " + ops.toFixed(3) + " vs " + (oppHand === "L" ? "LHP" : "RHP")
  };
}
...
function formFactor(era, fip) {
  ...
  if (metric == null) return {
    f: 1,
    note: "recent form n/a"
  };
  const label = fip != null ? "FIP" : "ERA";
  return {
    f: nClamp(1 + (metric - 4.15) / 4.15 * 0.25, 0.85, 1.2),
...
function pitchSkillFactor(peri, lg) {
  if (!peri || !lg) return {
    f: 1,
    note: "peripherals n/a"
  };
  let f = 1;
  const dev = (v, base, w, dir) => {
    if (v != null && base) f *= nClamp(1 + dir * ((v - base) / base) * w, 0.88, 1.14);
  };
  ...
  const note = peri.k != null ? "K " + peri.k.toFixed(0) + "% · BB " + (peri.bb != null ? peri.bb.toFixed(0) : "-") + "% · barrel " + (peri.barrel != null ? peri.barrel.toFixed(0) : "-") + "% · GB " + (peri.gb != null ? peri.gb.toFixed(0) : "-") + "%" : "n/a";
  return {
    f: nClamp(f, 0.80, 1.20),
    note
  };
}
...
function openerGameFactor(meta) {
  if (!meta || meta.gs == null || meta.g == null) return {
    f: 1,
    note: "",
    opener: false
  };
  ...
  return opener ? {
    f: 0.93,
    note: "likely opener/bullpen game",
    opener: true
  } : {
    f: 1,
    note: "starter",
...
function openerFactor(i01Era, seasonEra) {
  if (i01Era == null || seasonEra == null || seasonEra <= 0) return {
    f: 1,
    note: "n/a"
  };
  ...
  return {
    f: nClamp(1 + (ratio - 1) * 0.15, 0.9, 1.12),
    note: "1st-inn " + i01Era.toFixed(2) + " vs " + seasonEra.toFixed(2) + " ERA (" + tag + ")"
  };
}
...
function seasonLoadFactor(ip) {
  ...
  if (ip >= 120) return {
    f: 1.01,
    note: Math.round(ip) + " IP (building load)"
  };
  return {
    f: 1,
    note: ""
...
async function topOrderStrength(players, season, oppHand, oppPitcherId) {
  ...
  try {
    ...
    const d = await getJson("https://statsapi.mlb.com/api/v1/people?personIds=" + ordered.join(",") + "&hydrate=stats(group=[hitting]," + type + ",season=" + season + ")");
    const byId = {};
    (d.people || []).forEach(p => {
      const s = p.stats && p.stats[0] && p.stats[0].splits && p.stats[0].splits[0] && p.stats[0].splits[0].stat;
      if (s) byId[p.id] = {
        obp: s.obp != null ? Number(s.obp) : null,
        rates: paRates(s, s.plateAppearances)
      };
    });
    const w = [0.5, 0.3, 0.2];
    let num = 0,
      den = 0;
    ordered.slice(0, 3).forEach((id, i) => {
      const o = byId[id] && byId[id].obp;
      if (o != null) {
        num += o * w[i];
        den += w[i];
      }
    ...
    if (den > 0) {
      const obp = num / den;
      val = {
        factor: nClamp(obp / NRFI_LG_OBP, 0.82, 1.24),
        obp,
        batters: hasB ? batters : null,
...
async function travelRest(teamId, todayStr, venueId) {
  ...
  try {
    ...
    const d = await getJson("https://statsapi.mlb.com/api/v1/schedule?sportId=1&teamId=" + teamId + "&startDate=" + start + "&endDate=" + todayStr + "&hydrate=venue");
    const games = [];
    (d.dates || []).forEach(dt => (dt.games || []).forEach(g => games.push({
      date: dt.date,
      g
    })));
    ...
    if (prev) {
      ...
      if (restDays <= 1 && traveled) val = {
        factor: 0.93,
        note: "played yesterday + traveled"
      };else if (restDays <= 1) val = {
        factor: 0.98,
        note: "played yesterday"
      };else if (restDays >= 3) val = {
        factor: 1.03,
        note: restDays + " days rest"
      };else val = {
        factor: 1,
        note: restDays + "d rest"
      };
    }
  } catch {/* leave neutral */}
...
function matchupPA(b, p, lg) {
  ...
  let sum = 0;
  for (const k of keys) {
    const v = lg[k] > 0 ? b[k] * p[k] / lg[k] : 0;
    raw[k] = v;
    sum += v;
  }
  if (sum <= 0) return Object.assign({}, lg);
  const o = {};
  for (const k of keys) o[k] = raw[k] / sum;
  return o;
}
...
function nrfiEvaluate(ctx) {
  ...
  const awayRestDays = gameDate && ctx.awayMeta && ctx.awayMeta.lastStartDate ? Math.round((new Date(gameDate) - new Date(ctx.awayMeta.lastStartDate)) / 86400000) : null;
  const homeRestDays = gameDate && ctx.homeMeta && ctx.homeMeta.lastStartDate ? Math.round((new Date(gameDate) - new Date(ctx.homeMeta.lastStartDate)) / 86400000) : null;
  ...
  const pNRFI = dayGameShift > 0 ? nClamp(unlogit(logit(nClamp(rawNRFI, 0.02, 0.98)) - dayGameShift), 0.02, 0.98) : rawNRFI;
  ...
  const checks = [{
    ...
    lean: awayRest.f >= 1.05 || homeRest.f >= 1.05 ? "yrfi" : "neutral"
  } : null, isDayGame ? {
    label: "Day game",
...
function nrfiVerdict(r) {
  ...
  if (total >= 3 && frac < 0.5 && strength !== "PASS") {
    strength = down(strength, 1);
    notes.push("signals split");
  }
  ...
  if (conf < 0.35) {
    strength = "PASS";
    notes.push("thin data");
  } else if (conf < 0.55 && (strength === "STRONG" || strength === "BET")) {
    strength = "LEAN";
    notes.push("limited data");
  }
  // STRONG demands both high confidence AND strong agreement.
  if (strength === "STRONG" && !(conf >= 0.7 && frac >= 0.6)) {
    strength = "BET";
    notes.push("not full confidence");
  }
  ...
  if (r.market) {
    ...
    if (edge == null) {
      strength = "PASS";
      notes.push("game under way — no pregame edge left");
    } else if (edge < 2) {
      strength = "PASS";
      notes.push("market efficient — no value");
    } else if (mktProb >= 62 && edge < 5) {
      strength = "PASS";
      notes.push("juice too short");
    } else if ((strength === "STRONG" || strength === "BET") && edge < 3) {
      strength = down(strength, 1);
  ...
  const blurb = (strength === "PASS" ? outcome + " only " + p.toFixed(0) + "% — basically a coin flip." + al : outcome + " — " + p.toFixed(0) + "% (" + word + " lean)." + al) + tail;
  return {
    strength,
    side,
    isBet,
...
async function scanNrfi(onProgress) {
  ...
  const date = today();
  const [sch, whiffRes, umpRes] = await Promise.all([getJson("https://statsapi.mlb.com/api/v1/schedule?sportId=1&date=" + date + "&hydrate=probablePitcher,linescore,team,lineups,weather,venue,officials"), fetch("/api/desk/savant").then(r => r.json()).catch(() => null), fetch("/api/desk/umpires").then(r => r.json()).catch(() => null)]);
  const periById = whiffRes && whiffRes.byId || {};
  const lg = whiffRes && whiffRes.lg || {
    k: 22,
    bb: 8,
    barrel: 7.5,
    gb: 44,
  ...
  const rows = await mapLimit(games, 4, async g => {
    ...
    const ctx = {
      ...
      awayPPId: awayPP && awayPP.id,
      homePPId: homePP && homePP.id,
      awayOff,
      homeOff,
      awayPit,
      homePit,
      ...
      lg,
      umpName: umpName || null,
      umpFactor: umpEntry ? umpEntry.runFactor : 1,
      umpNote: umpEntry ? umpEntry.note : "",
    ...
    return {
      ...
      away: ctx.awayName,
      home: ctx.homeName,
      awayAbbr: away && away.team && away.team.abbreviation,
      homeAbbr: home && home.team && home.team.abbreviation,
      awayPP: ctx.awayPP,
      homePP: ctx.homePP,
      pNRFI: ev.pNRFI,
      pYRFI: 1 - ev.pNRFI,
...
function FirstInning() {
  ...
  async function loadOpenPositions() {
    ...
    try {
      ...
      if (d.positions) setOpenPositions(d);else setOpenPositions({
        error: d.error || "Failed to load",
        positions: []
      });
    } catch {
      setOpenPositions({
  ...
  async function run() {
    ...
    try {
      const [r, rfiList] = await Promise.all([scanNrfi((done, total) => setProg({
        done,
        total
      })), fetchKalshiRFI()]);
  ...
  const enriched = rows.map(r => {
    ...
    if (mk) {
      ...
      market = {
        ...
        marketSide,
        edge: started ? null : modelSide - marketSide,
        mktMove,
        started
      };
    }
  ...
  const card = r => {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, vg.tag)), (avgPitHP != null || avgOffHP != null) && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: 8
    ...
    }, "\uD83D\uDEE1\uFE0F PITCHING"), avgPitHP != null && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 10,
        fontWeight: 800,
    ...
    }, avgPitHP >= 65 ? "elite — starters shutting it down" : avgPitHP >= 50 ? "solid — starters in control" : "shaky — vulnerable to early runs")), /*#__PURE__*/React.createElement("div", {
      style: {
        textAlign: "center",
        flexShrink: 0
      }
      ...
      style: {
        fontSize: 7,
        fontWeight: 800,
        color: pitWinning === true ? "var(--moss)" : pitWinning === false ? "var(--rose)" : "var(--dim)",
        letterSpacing: "0.04em",
        marginTop: 2,
        whiteSpace: "nowrap"
      }
    }, pitWinning === true ? "ARM WINS" : pitWinning === false ? "BAT WINS" : "TOSS UP")), /*#__PURE__*/React.createElement("div", {
    ...
    }, avgOffHP >= 60 ? "hot bats — scoring threat is real" : avgOffHP >= 40 ? "average — could go either way" : "cold lineup — quiet early innings"))))), r.pitProfiles && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        style: {
          ...
          marginTop: 5,
          padding: "2px 7px",
          background: btBadge.bg,
          border: "1px solid " + btBadge.border,
          borderRadius: 5,
          fontSize: 10,
          fontWeight: 700,
          color: btBadge.color
      ...
      title: r.market.edge == null ? "No edge is quoted once a game is under way: the market is pricing the inning live while our number is still the pregame one, so the gap measures what the market already knows, not value." : "Edge = how much our model probability exceeds the market on our call side. " + (r.market.edge > 0 ? "Positive edge means we think the true probability is higher than what the market is paying." : "Negative edge means the market already prices this better than our model.") + " We only bet when edge is meaningfully positive.",
      style: {
        cursor: "help",
        padding: "8px 12px",
        borderRight: "1px solid rgba(255,255,255,0.06)"
    ...
    }, "EDGE"), r.market.edge == null ? /*#__PURE__*/React.createElement("span", {
      style: {
        fontWeight: 700,
        color: "var(--dim)"
      }
  ...
  const sect = (title, arr, color) => arr.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "panel",
    style: {
      marginTop: 12
    }
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
    ...
    return /*#__PURE__*/React.createElement("span", {
      ...
      title: "Model auto-refreshes every 10 minutes \u2014 keeps lineups, weather, market prices, and pitcher stats current"
    }, "Updated ", secsAgo < 60 ? secsAgo + "s ago" : Math.floor(secsAgo / 60) + "m ago", " \xB7 next in ", fmt(nextIn));
  })(), /*#__PURE__*/React.createElement("button", {
    className: "btn btn-ghost btn-sm",
    ...
    const allBetRows = enriched.filter(r => r.v && r.v.isBet && r.kelly != null && r.call === "NRFI").slice().sort((a, b) => (b.market ? b.market.edge : 0) - (a.market ? a.market.edge : 0));
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, gradedHistory.length > 0 && /*#__PURE__*/React.createElement("span", {
      title: "Tracked P&L from " + gradedHistory.length + " settled Kalshi bet" + (gradedHistory.length === 1 ? "" : "s") + " in your history. This is informational — your bankroll is whatever you enter below.",
      style: {
        cursor: "help",
        fontSize: 12,
        color: historyPL >= 0 ? "var(--moss)" : "var(--rose)",
        fontWeight: 700
      }
    }, "History P&L: ", historyPL >= 0 ? "+" : "", "$", historyPL.toFixed(2)), /*#__PURE__*/React.createElement("button", {
      onClick: async () => {
        ...
        try {
          ...
          if (d.balance != null) {
            ...
            setSyncMsg({
              ok: true,
              text: "Synced: $" + v.toFixed(2)
            });
          } else {
            setSyncMsg({
              ok: false,
              text: d.error || "Could not fetch balance"
            });
          }
        } catch {
          setSyncMsg({
      ...
      style: {
        ...
        borderRadius: 6,
        color: "var(--moss)",
        cursor: syncingBalance ? "not-allowed" : "pointer",
        fontWeight: 700,
        opacity: syncingBalance ? 0.6 : 1
      }
    }, syncingBalance ? "Syncing…" : "↻ Sync Kalshi Balance"), syncMsg && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 11,
        color: syncMsg.ok ? "var(--moss)" : "var(--rose)"
      }
    }, syncMsg.text))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: 14,
        flexWrap: "wrap",
    ...
    }, "+$", (remaining * effectiveDailyEv).toFixed(0)))), betRows.length === 0 && alreadyHeld.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: 10,
        padding: "10px 14px",
    ...
    }, "\u2713 All ", alreadyHeld.length, " model picks are already in your open positions \u2014 nothing left to place today."), betRows.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: 10
      }
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u2713 ", alreadyHeld.length, " already placed \u2014 see open positions below"), allocationScale < 1 && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 10,
        color: "var(--amber)"
      }
    ...
    }, "OPEN KALSHI POSITIONS"), /*#__PURE__*/React.createElement("button", {
    ...
    }, openPositions.error), openPositions && !openPositions.error && openPositions.positions.length === 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "var(--dim)",
        padding: "6px 10px"
      }
    }, "No open NRFI positions found."), openPositions && !openPositions.error && openPositions.positions.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexDirection: "column",
        gap: 5
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        style: {
          ...
          fontSize: 11,
          fontWeight: 800,
          background: p.call === "NRFI" ? "rgba(80,160,80,0.15)" : "rgba(220,60,60,0.15)",
...
function Parlay({
  ...
  const shown = useMemo(() => {
    ...
    return dedupe(picks, (p, c) => p.modelProb > c.modelProb).sort((a, b) => b.modelProb - a.modelProb).slice(0, 60);
  }, [picks, view, minEdge]);
